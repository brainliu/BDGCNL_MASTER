...
def MAPE_torch(pred, true, mask_value=None):
    if mask_value != None:
        mask = torch.gt(true, mask_value) #比较true和mask value的值，如果前面大于后面则取1，否则变为0
        pred = torch.masked_select(pred, mask) #就是选择mask中为1的元素的值，保存为1列比较厉害
        true = torch.masked_select(true, mask)
